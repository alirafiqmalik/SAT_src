...
bool solver_t::_get_distinguishing_input()
{
    using namespace sat_n;
    using namespace ckt_n;
    bool cand_result = Scand.guro_solve();

    assert (cand_result == true);
    cex_assumptions[0] = l_out;
    for (unsigned i=0; i != ckt.num_key_inputs(); i++) {
        lbool lki = Scand.guro_modelValue(candidate_keyinput_literals[i]);
        assert (lki.isDef());
        if (lki.getBool()) {
    ...
    bool result = Scex.guro_solve(cex_assumptions);
    ...
    for(unsigned i=0; i != dbl.dbl->num_ckt_inputs(); i++) {
        int jdx  = dbl.dbl->ckt_inputs[i]->get_index();
        lbool val = Scex.guro_modelValue(lmap[jdx]);
        assert(val.isDef());
        if(!val.getBool()) {
...
bool solver_t::_appsat_random_query()
{
    ...
    bool sat = Scand.guro_solve(cand_assumptions);
    ...
    for(unsigned i=0; i != output_values.size(); i++) {
        ...
        lbool ri = Scand.guro_modelValue(candidate_output_literals[i]);
        if (verbose) {
            if (!ri.isDef()) {
                std::cout << "-";
            } else {
...
void solver_t::_sanity_check_model()
{
    ...
    if(Scex.guro_solve(assumps) == false) {
        std::cout << "UNSAT result during sanity check." << std::endl;
        std::cout << "result of no-assumption solve: " << Scex.guro_solve() << std::endl;
        exit(1);
    }

    for(unsigned i=0; i != output_values.size(); i++) {
        bool vi = output_values[i];
        lbool ri = Scex.guro_modelValue(output_literals_A[i]);
        if(!(ri.isDef() && ri.getBool() == vi)) { 
            pass = false;
        }
    }
...
int solver_t::_verify_solution_sim(rmap_t& keysFound)
{
    ...
    for(int iter=0; iter < MAX_VERIF_ITER;  iter++) {
        ...
        if(Scand.guro_solve(assumps) == false) {
...
void solver_t::_extractSolution(rmap_t& keysFound)
{
    ...
    for(unsigned i=0; i != keyinput_literals_A.size(); i++) {
        lbool v = Scand.guro_modelValue(candidate_keyinput_literals[i]);
        if(!v.getBool()) {
